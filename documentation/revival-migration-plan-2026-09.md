...
**Updated:** 2026-09-23
**Status:** Offline 30-minute and 5-minute reconstructions working; live simulated decision loop still to build.
...
2. A dedicated v2 simulation runner creates exactly five persistent, independently resettable 50K Combine ledgers. Use the familiar names alpha, beta, gamma, delta, and epsilon for the experiment, with explicit strategy/cadence/risk settings for each. Historical alpha/beta/gamma were 5-minute, delta 15-minute, and epsilon 30-minute; preserve those as the first comparison candidates, then change settings only through versioned experiments.
...
The offline prototype already models five generic risk settings to exercise the rules. Those settings are **not** the final alpha-to-epsilon configuration. A pass in this simulator should create a reviewable “eligible for XFA simulation” event; a separate XFA ledger begins at $0 only after that stage is deliberately enabled.
...
2. **Persistent simulated broker.** Turn the current offline replay rules into an event-backed runner on the Pi, with five independent ledgers, restart recovery, duplicate-bar protection, a manual pause, and account reset/pass/fail transitions. Add an XFA ruleset later as a separate state machine. Exit when a restart, duplicate alert, missing bar, stop/target collision, MLL touch, session close, and consistency-target scenario all produce the expected ledger state.
3. **ProDex decision-only loop.** Create a dedicated simulation workflow in the Pi n8n instance, using the current epsilon ProDex connection and overseer prompt as a starting specification. Do not reuse the old `ai_trading_log` or `charts` write nodes as the simulation ledger, and do not call the legacy broker path. Feed v2-generated snapshots and each simulated account's position context. Persist every request and response with an idempotency key. Start with a forward shadow run before any performance interpretation. Keep the five account ledgers and variant settings separate.
4. **Forward shadow operation and comparison.** Run the five simulated accounts from fresh bars without any broker route. Compare each variant on the same calendar windows using net results after fees, drawdown, pass rate, best-day concentration, and data-exclusion rate. Check daily that Supabase bars, n8n decisions, and v2 ledger events reconcile. Use chronological train/validation/test windows; do not optimize settings on the final holdout window. Exit when the loop survives restarts and missed responses, account status is explainable from ledger events, and forward results are long enough to judge beyond a few isolated trades.
